...
export const STICKER_IMAGES = Object.fromEntries(Object.keys(stk).map(k => [k.split('/').pop().replace('.png', ''), stk[k]]))
...
const kawaii = import.meta.glob('./assets/kawaii/*.png', { eager: true, import: 'default' })
export const KAWAII_IMAGES = Object.fromEntries(Object.keys(kawaii).map(k => [k.split('/').pop().replace('.png', ''), kawaii[k]]))
export const KAWAII = Object.keys(kawaii).sort().map(k => kawaii[k])
